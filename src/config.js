...
export const TOKENS = Object.freeze({
  ...Object.fromEntries(ASSETS.map(({ symbol, mint, decimals }) => [symbol, { mint, decimals }])),
...
export const TRACKED_ASSETS = Object.freeze(ASSETS.map(asset => asset.symbol));
...
export function config(env = process.env, requireTelegram = true) {
  ...
  return {
    ...
    maxTrade: amount(inverse ? 'MAX_TRADE_USDC' : 'MAX_TRADE_SOL', inverse ? '1' : '0.1', quoteDecimals),
